...
impl App for DemoApp {
    ...
    fn render(&mut self, time: f32) -> Result<()> {
        let background_color: [GLfloat; 4] =
            [(time.sin() * 0.5) + 0.5, (time.cos() * 0.5) + 0.5, 0.0, 1.0];

        let triangle_color: [GLfloat; 4] =
            [(time.sin() * 0.5) + 0.5, (time.cos() * 0.5) + 0.5, 1.0, 1.0];

        let offset: [GLfloat; 4] = [time.sin() * 0.5, time.cos() * 0.6, 0.0, 0.0];
...
fn main() -> Result<()> {
    ...
    run_application(app, "Colored Triangle")
}
